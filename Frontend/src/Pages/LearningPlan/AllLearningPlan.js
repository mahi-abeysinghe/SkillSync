...
function AllLearningPlan() {
  ...
  const handleSearch = (e) => {
    ...
    // Filter posts based on title, description, category, or owner name
    const filtered = posts.filter(
      (post) =>
        post.title.toLowerCase().includes(query) ||
        post.description.toLowerCase().includes(query) ||
        (post.category && post.category.toLowerCase().includes(query)) ||
        post.postOwnerName.toLowerCase().includes(query)
    );
        ...
        <div className={`continSection ${appear ? 'appear' : ''}`}>
          <div className='search-container'>
            ...
            <input
              ...
              placeholder="Search by title, description, category, or owner name"
              value={searchQuery}
